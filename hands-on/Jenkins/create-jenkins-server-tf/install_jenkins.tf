...
resource "null_resource" "forpasswd" {
  ...
  provisioner "local-exec" {
    command = "ssh -i ~/OneDrive/${var.mykey}.pem -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ec2-user@${aws_instance.tf-jenkins-server.public_ip} 'sudo cat /var/lib/jenkins/secrets/initialAdminPassword' > initialpasswd.txt"
  }
}
